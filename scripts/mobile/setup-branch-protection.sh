...
# and the NAPI symbol smoke). Job names must match exactly; matrix jobs
# (build-*, combine-android) are intentionally NOT listed — the single-instance
# smoke-* jobs transitively require them via `needs:`.
cat > /tmp/branch-protection.json <<'JSON'
{
  "required_status_checks": {
    "strict": true,
    "contexts": ["smoke-android", "smoke-ios", "smoke-host", "napi-smoke-android"]
  },
...
echo "Done. Required checks: smoke-android, smoke-ios, smoke-host, napi-smoke-android."
