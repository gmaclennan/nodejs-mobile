...
binary) and `lite` (comapeo-tuned; see
[`BINARY_SIZE_REDUCTION.md`](./BINARY_SIZE_REDUCTION.md)).
...
3. **Wait for green.** Required checks (build + smoke + NAPI) must pass to merge;
   ...
   release commit.
4. **Real-device smoke** (RELEASE_PLAN Phase 5 / Tier 3) — load the artifacts on
   physical hardware, including a NAPI addon, before approving.
